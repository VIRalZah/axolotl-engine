...
static void CopyOrSwap(const uint32_t* src, int num_pixels, uint8_t* dst,
  ...
  if (is_big_endian() == swap_on_big_endian) {
    ...
    while (src < src_end) {
      uint32_t argb = *src++;
#if (!defined(__BIG_ENDIAN__) && (defined(__i386__) || defined(__x86_64__))) && (AX_TARGET_PLATFORM != AX_PLATFORM_MARMALADE)
      __asm__ volatile("bswap %0" : "=r"(argb) : "0"(argb));
      *(uint32_t*)dst = argb;
      dst += sizeof(argb);
#elif (!defined(__BIG_ENDIAN__) && defined(_MSC_VER)) && (AX_TARGET_PLATFORM != AX_PLATFORM_MARMALADE)
