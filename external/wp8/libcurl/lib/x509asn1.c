                             ...
                             const char * oid)
{
  ...
  Point = Curl_getASN1Element(&e, beg, end);
  if(!Point || e.tag != CURL_ASN1_OBJECT_IDENTIFIER)
    return (const char *) NULL;
  ...
  free((char *) p);
  return matched? Point: (const char *) NULL;
}
                    ...
                    const char * beg, const char * end)
{
  ...
  beg = Curl_getASN1Element(&elem, beg, end);
  Point = Curl_getASN1Element(&cert->notBefore, elem.beg, elem.end);
  Curl_getASN1Element(&cert->notAfter, Point, elem.end);
  /* Get subject. */
  beg = Curl_getASN1Element(&cert->subject, beg, end);
  /* Get subjectPublicKeyAlgorithm and subjectPublicKey. */
  beg = Curl_getASN1Element(&elem, beg, end);
  Point = Curl_getASN1Element(&cert->subjectPublicKeyAlgorithm,
                            elem.beg, elem.end);
  Curl_getASN1Element(&cert->subjectPublicKey, Point, elem.end);
                               ...
                               const char * end)
{
  ...
  /* Subject. */
  Point = Curl_DNtostr(&cert.subject);
  if(!Point)
    return CURLE_OUT_OF_MEMORY;
  if(data->set.ssl.certinfo)
    Curl_ssl_push_certinfo(data, certnum, "Subject", Point);
  if(!certnum)
    infof(data, "%2d Subject: %s\n", certnum, Point);
  free((char *) Point);

  /* Issuer. */
  Point = Curl_DNtostr(&cert.issuer);
  if(!Point)
    return CURLE_OUT_OF_MEMORY;
  if(data->set.ssl.certinfo)
    Curl_ssl_push_certinfo(data, certnum, "Issuer", Point);
  if(!certnum)
    infof(data, "   Issuer: %s\n", Point);
  free((char *) Point);
  ...
  version = 0;
  for(Point = cert.version.beg; Point < cert.version.end; Point++)
    version = (version << 8) | *(const unsigned char *) Point;
  if(data->set.ssl.certinfo) {
    Point = curl_maprintf("%lx", version);
    if(!Point)
      return CURLE_OUT_OF_MEMORY;
    Curl_ssl_push_certinfo(data, certnum, "Version", Point);
    free((char *) Point);
  }
  if(!certnum)
  ...
  /* Serial number. */
  Point = Curl_ASN1tostr(&cert.serialNumber, 0);
  if(!Point)
    return CURLE_OUT_OF_MEMORY;
  if(data->set.ssl.certinfo)
    Curl_ssl_push_certinfo(data, certnum, "Serial Number", Point);
  if(!certnum)
    infof(data, "   Serial Number: %s\n", Point);
  free((char *) Point);

  /* Signature algorithm .*/
  Point = dumpAlgo(&param, cert.signatureAlgorithm.beg,
                 cert.signatureAlgorithm.end);
  if(!Point)
    return CURLE_OUT_OF_MEMORY;
  if(data->set.ssl.certinfo)
    Curl_ssl_push_certinfo(data, certnum, "Signature Algorithm", Point);
  if(!certnum)
    infof(data, "   Signature Algorithm: %s\n", Point);
  free((char *) Point);

  /* Start Date. */
  Point = Curl_ASN1tostr(&cert.notBefore, 0);
  if(!Point)
    return CURLE_OUT_OF_MEMORY;
  if(data->set.ssl.certinfo)
    Curl_ssl_push_certinfo(data, certnum, "Start Date", Point);
  if(!certnum)
    infof(data, "   Start Date: %s\n", Point);
  free((char *) Point);

  /* Expire Date. */
  Point = Curl_ASN1tostr(&cert.notAfter, 0);
  if(!Point)
    return CURLE_OUT_OF_MEMORY;
  if(data->set.ssl.certinfo)
    Curl_ssl_push_certinfo(data, certnum, "Expire Date", Point);
  if(!certnum)
    infof(data, "   Expire Date: %s\n", Point);
  free((char *) Point);

  /* Public Key Algorithm. */
  Point = dumpAlgo(&param, cert.subjectPublicKeyAlgorithm.beg,
                 cert.subjectPublicKeyAlgorithm.end);
  if(!Point)
    return CURLE_OUT_OF_MEMORY;
  if(data->set.ssl.certinfo)
    Curl_ssl_push_certinfo(data, certnum, "Public Key Algorithm", Point);
  if(!certnum)
    infof(data, "   Public Key Algorithm: %s\n", Point);
  do_pubkey(data, certnum, Point, &param, &cert.subjectPublicKey);
  free((char *) Point);

/* TODO: extensions. */

  /* Signature. */
  Point = Curl_ASN1tostr(&cert.signature, 0);
  if(!Point)
    return CURLE_OUT_OF_MEMORY;
  if(data->set.ssl.certinfo)
    Curl_ssl_push_certinfo(data, certnum, "Signature", Point);
  if(!certnum)
