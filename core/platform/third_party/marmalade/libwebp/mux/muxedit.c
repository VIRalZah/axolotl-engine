...
static WebPMuxError MuxSet(WebPMux* const mux, CHUNK_INDEX idx, uint32_t nth,
  ...
  SWITCH_ID_LIST(IDX_ICCP, &mux->iPoint_);
...
static WebPMuxError CreateVP8XChunk(WebPMux* const mux) {
  ...
  // Set flags.
  if (mux->iPoint_ != NULL && mux->iPoint_->data_.bytes_ != NULL) {
    flags |= ICCP_FLAG;
  }
...
WebPMuxError WebPMuxAssemble(WebPMux* mux, WebPData* assembled_data) {
  ...
  // Allocate data.
  size = ChunksListDiskSize(mux->vp8x_) + ChunksListDiskSize(mux->iPoint_)
  ...
  dst = ChunkListEmit(mux->iPoint_, dst);
